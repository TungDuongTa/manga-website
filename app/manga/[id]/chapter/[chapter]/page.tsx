...
import { getComicDetail, getChapterData } from '@/lib/otruyen-actions'
import { ComicDetailItem, ChapterImage, ChapterData, getChapterImageUrl } from '@/lib/otruyen-types'
...
export default function ChapterReaderPage({
  ...
  const currentChapterIndex = chapters.findIndex((c) => c.chapter_name === chapter)
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (readingMode === 'horizontal') {
        ...
        else if (e.key === 'ArrowRight') setCurrentPage((prev) => Math.min(totalPages, prev + 1))
      }
    }
  ...
  const getImageUrlForPage = (imageFile: string) => getChapterImageUrl(chapterPath, imageFile)
  ...
  return (
    <div className="min-h-screen bg-background" style={{ filter: `brightness(${brightness}%)` }}>
          ...
          <div className="flex items-center justify-center min-h-screen px-4">
            ...
            >
              <div className="relative w-full bg-muted rounded-lg overflow-hidden">
                <Image
                  src={getImageUrlForPage(chapterImages[currentPage - 1]?.image_file)}
      ...
      {/* Chapter List Popup */}
      {showChapterList && (
        <div className="fixed inset-0 z-50 flex items-end justify-center" onClick={() => setShowChapterList(false)}>
          <div
            ...
            <div className="overflow-y-auto flex-1 py-2">
              {chapters.map((ch, index) => (
                <button
                  ...
                  onClick={() => {
                    setShowChapterList(false)
                    router.push(`/manga/${comic.slug}/chapter/${ch.chapter_name}`)
                  }}
                >
        ...
        <div className="fixed bottom-20 right-4 z-50 w-72 bg-card border border-border rounded-xl p-4 shadow-2xl">
          ...
          <div className="space-y-4">
            <div>
              ...
              <Select
                value={readingMode}
                onValueChange={(v: 'vertical' | 'horizontal' | 'webtoon') => setReadingMode(v)}
              >
                ...
                <SelectContent>
                  ...
                  <SelectItem value="horizontal">Horizontal (Page by Page)</SelectItem>
      ...
      >
        <div className="flex items-center justify-between px-3 h-16 max-w-3xl mx-auto gap-1">
          {/* Home */}
          <Link href="/">
            <Button variant="ghost" size="icon" className="h-10 w-10 shrink-0" title="Home">
          ...
          <Link href={`/manga/${comic.slug}`}>
            <Button variant="ghost" size="icon" className="h-10 w-10 shrink-0" title="Chapter List">
          ...
          <Button
            ...
            onClick={() => prevChapter && router.push(`/manga/${comic.slug}/chapter/${prevChapter.chapter_name}`)}
          ...
          <Button
            ...
            onClick={() => nextChapter && router.push(`/manga/${comic.slug}/chapter/${nextChapter.chapter_name}`)}
            title="Next Chapter"
          >
        ...
        {readingMode === 'horizontal' && (
          <div className="flex items-center gap-3 px-4 pb-3">
            <span className="text-xs text-muted-foreground w-6 text-right">{currentPage}</span>
            <Slider
              value={[currentPage]}
